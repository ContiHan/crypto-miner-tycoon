...
/// equip at most [KeystoneSystem.equipCap] (owner: 2), and only from a TECH branch
/// whose CAPSTONE node you've researched (they live at each branch capstone).
/// Effects are declared as neutral-default modifier fields; GameLogic folds the
/// equipped ones into a [KeystoneModifiers] aggregate at the consumption sites.
class KeystoneDef {
  final String id;
  final String name;
  final String description;
  final String branch; // 'A' Foundry / 'B' Golden Nonce / 'C' Degen
...
/// The 12 keystones (4 per branch — unlocked by owning that branch's capstone).
/// Each = one bounded lever + a symmetric cost. Numbers from CHAOS_DEPTH §3.
...
class KeystoneSystem {
  ...
  List<KeystoneDef> availableFor(Set<String> ownedCapstones) =>
      kKeystones.where((k) => ownedCapstones.contains(k.branch)).toList();
  ...
  List<KeystoneDef> availableOrEquipped(Set<String> ownedCapstones) {
    final out = availableFor(ownedCapstones);
  ...
  bool toggle(String id, Set<String> ownedCapstones) {
    if (equipped.contains(id)) {
      equipped.remove(id);
      return false;
    }
    final def = byId(id);
    if (def == null || !ownedCapstones.contains(def.branch)) return false;
